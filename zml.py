...
def bacaData():
	...
	try:
		fid_bgroup = open(os.sys.path[0]+'/ZMLgroup.txt','r').readlines()
	except:pass
	try:
		fid_bteman = open(os.sys.path[0]+'/ZMLteman.txt','r').readlines()
	except:pass
def simpan():
	if len(id_bgroup) != 0:
		cetak('!h[*] Menyimpan hasil dari Group')
		try:
			open(os.sys.path[0]+'/ZMLgroup.txt','w').write('\n'.join(id_bgroup))
			cetak('!h[!] Berhasil meyimpan !cZMLgroup.txt')
		except:
			cetak('!m[!] Gagal meyimpan')
	if len(id_bteman) != 0:
		cetak('!h[*] Menyimpan hasil daftar Teman...')
		try:
			open(os.sys.path[0]+'/ZMLteman.txt','w').write('\n'.join(id_bteman))
			cetak('!h[!] Berhasil meyimpan !cZMLteman.txt')
		except:
			cetak('!m[!] Gagal meyimpan')
def keluar():
	simpan()
...
def lanjutG():
	...
	if len(fid_bgroup) != 0:
		...
		if i.upper() == 'L':
			...
			os.remove(os.sys.path[0]+'/ZMLgroup.txt')
			fid_bgroup = []
	return 0
def lanjutT():
	global fid_bteman
	if len(fid_bteman) != 0:
		...
		if i.upper() == 'L':
			...
			os.remove(os.sys.path[0]+'/ZMLteman.txt')
			fid_bteman = []
	return 0
def buka(d):
	try:
...
def idteman():
	...
	if i.upper() == 'Y':
		...
		return menu()
def menu():
	cetak("\n           !h.-.-..\n          /+/++//\n         /+/++//\n  !k*   !k* !h/+/++//\n   \ /  |/__//\n !h{!mX!h}v{!mX!h}!0!b|!cZML!b|==========.\n   !h(!m'!h)!0  !h/'|'\           !b\\\n       !h/  \  \          !b'\n       !h\_  \_ \_   !k___!mZML!kZamuel Voldemord\n !m* !hGithub !m: !mhttps://github.com/zamxyz\n !m* !hFacebook !m: !mFacebook.com/zamuel.mx.01\n!k.======================.\n|!h  AMBIL !mID!h DARI.....  !k|\n'======================'\n!m#!p1 !hDAFTAR TEMAN\n!m#!p2 !hANGGOTA GROUP\n!m#!p3 !mID PUBLIK\n!m#!p4 !mKELUAR...")
